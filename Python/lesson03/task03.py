...
class BST:
    ...
    def WideAllNodes(self):
        ...
        level = result.copy()
        while level:
            level = self.get_children_for_level(level)
            result.extend(level)
        return tuple(result)

    def get_children_for_level(self, level):
        next_level = []
        if not level:
            return next_level
        for node in level:
    ...
    def DeepAllNodes(self, traversal_type):
        result = []
        self.get_deep_in_order(self.Root, result, traversal_type)
        return tuple(result)

    def get_deep_in_order(self, current_node, nodes, traversal_type):
        if current_node is None:
            return
        if traversal_type == 2:
            nodes.append(current_node)
        self.get_deep_in_order(current_node.LeftChild, nodes, traversal_type)
        if traversal_type == 0:
            nodes.append(current_node)
        self.get_deep_in_order(current_node.RightChild, nodes, traversal_type)
        if traversal_type == 1:
            nodes.append(current_node)
